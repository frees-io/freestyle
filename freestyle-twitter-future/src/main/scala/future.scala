...
object future {
  ...
  object implicits {
    ...
    object nondeterminism {
      ...
      implicit val freestyleParallelTwitterFutureInstances: MonadError[Future, Throwable] with Capture[Future] = new FreestyleTwitterMonad {

        override def ap[A, B](ff: Future[A => B])(fa: Future[A]): Future[B] =
          fa.join(ff).map { case (a, f) => f(a) }
