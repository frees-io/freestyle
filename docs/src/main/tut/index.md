...
Freestyle includes all the implicit machinery necessary to achieve seamless dependency injection of `@free` and `@module` Algebras.
...
In addition Freestyle includes commonly used FP efects stack such as `option`, `error`, `reader`, `writer`, `state` based on the capabilities of
