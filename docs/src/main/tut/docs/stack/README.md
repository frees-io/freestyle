...
As a target type of our computation, we choose a combination of `Kleisli` and `cats.effect.IO`.

- [`Kleisli`](http://typelevel.org/cats/datatypes/kleisli.html) or `ReaderT` is like an `A => F[B]` function, where `A` can be seen as an [environment or configuration](http://typelevel.org/cats/datatypes/kleisli.html#configuration) type. In this case, `Stack` can be seen as `Config => IO[A]`,
- [`cats.effect.IO`](https://oss.sonatype.org/service/local/repositories/releases/archive/org/typelevel/cats-effect_2.12/0.4/cats-effect_2.12-0.4-javadoc.jar/!/cats/effect/IO.html) is a data type that can be used for potentially lazy computations which can contain asynchronous steps. The `IO` implementation used in this case is from the [cats-effect library](https://github.com/typelevel/cats-effect), similar data types can be found in [Monix](https://github.com/monix/monix) ([`Monix.eval.Task`](https://monix.io/docs/2x/eval/task.html)) and [Scalaz](https://github.com/scalaz/scalaz) ([`scalaz.concurrent.Task`](http://timperrett.com/2014/07/20/scalaz-task-the-missing-documentation/)).

We use `Kleisli` to fulfil the `ReaderM` constraint and `IO` the `ErrorM` constraint:
...
implicit val customerPersistencteHandler: CustomerPersistence.Handler[Stack] =
  new CustomerPersistence.Handler[Stack] {
    ...
    def getCustomer(id: CustomerId): Stack[Option[Customer]] =
      Kleisli(_ => IO.pure(customers.get(id)))
  }
...
implicit val stockPersistencteHandler: StockPersistence.Handler[Stack] =
  new StockPersistence.Handler[Stack] {
    ...
    def registerOrder(order: Order): Stack[Unit] =
      Kleisli(_ => IO(println(s"Register $order")))
  }
```
...
We can run our `Stack` by supplying a `Config` value and running the `IO`.

By supplying a `Config` value we can run our `Stack` which results in a `IO[String]`.
When we want to actually execute the program, we can run the `IO` by using one of the methods on `IO`:

```tut:book
val ioString: IO[String] = result.run(config)

println(ioString.unsafeRunSync)
```
