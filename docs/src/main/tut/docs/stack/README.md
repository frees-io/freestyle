...
When validating an order we need to get the information of the customer, if a customer places multiple orders we don't want to send a database request every time, so we can use the [`freestyle-cache`](../effects/cache) module to cache customers.
