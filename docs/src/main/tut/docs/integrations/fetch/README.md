...
Running data fetches optimized by the [Fetch](https://github.com/47deg/fetch) library in Freestyle programs can be achieved through the algebra and interpreter that are provided by the `frees-fetch` package. `frees-fetch` allows you to run Fetch computations when interpreting a free program, using the target runtime monad as their runtime type.
