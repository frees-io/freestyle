...
object error {
  ...
  object implicits {
    ...
    implicit def freeStyleErrorMInterpreter[M[_]](implicit ME: MonadError[M, Throwable]): ErrorM.Interpreter[M] = new ErrorM.Interpreter[M] {
    ...
    implicit def freeSLiftError[F[_]: ErrorM]: FreeSLift[F, Either[Throwable, ?]] = new ErrorFreeSLift[F]
