...
class TaglessTests extends WordSpec with Matchers {
  ...
  "the @tagless @stacksafe macro annotation should be rejected, and the compilation fail, if it is applied to" when {
    ...
    "an empty trait" in (
      "@tagless @stacksafe trait X" shouldNot compile
    )

    "an empty abstract class" in (
      "@tagless @stacksafe abstract class X" shouldNot compile
    )

    "a non-abstract class" in {
      "@tagless @stacksafe class X" shouldNot compile
    }

    "a trait with companion object" in {
      "@tagless @stacksafe trait X {def f: FS[Int]} ; object X" shouldNot compile
    }
    ...
    "a trait with any non-abstact methods of type FS" in {
      "@tagless @stacksafe trait X { def f(a: Char) : FS[Int] = 0 } " shouldNot compile
    }
