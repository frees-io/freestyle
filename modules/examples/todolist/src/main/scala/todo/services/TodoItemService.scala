...
trait TodoItemService {
  ...
  val model = classOf[TodoItem].getSimpleName
  ...
  def retrieve(id: Int): FS.Seq[Option[TodoItem]] =
    for {
  ...
  def insert(item: TodoItem): FS.Seq[Option[TodoItem]] =
    for {
  ...
  def update(item: TodoItem): FS.Seq[Option[TodoItem]] =
    for {
  ...
  def batchedInsert(items: List[TodoItem]): FS.Seq[List[Option[TodoItem]]] =
    for {
      _ <- log.debug(s"Trying to insert batch $model")
      r <- items.traverse(repo.insert)
    } yield r

  def batchedUpdate(items: List[TodoItem]): FS.Seq[List[Option[TodoItem]]] =
    for {
