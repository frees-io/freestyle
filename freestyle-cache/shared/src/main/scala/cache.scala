...
package cache {
  ...
  class KeyValueProvider[Key, Val] {
    ...
    @free sealed trait CacheM {
      ...
      def putAll(keyValues: Map[Key, Val]): Oper.Par[Unit]

      //If the specified key is not already associated with a value, associate it with the given value.
      def putIfAbsent(key: Key, newVal: Val): Oper.Par[Unit]
      ...
      def replace(key: Key, newVal: Val): Oper.Par[Unit]

      //Returns true if this cache contains no key-value mappings.
      def isEmpty: Oper.Par[Boolean]
